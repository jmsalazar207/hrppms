...
## Search 
$searchQuery = " WHERE userprofile.item_code !='' AND userprofile.empno !='' AND pds_personal_information.empno !='' ";
if($searchValue != ''){
...
while($Position_output = mysqli_fetch_array($Position_result)){
   ...
   $action = "<td class='center' style='text-align: center;'>
                    <button class='btn btn-primary btn-sm btnEdit' id='btnEdit' name = 'btnEdit' value = '$Item_Code' title='Modal View' onclick='modalEdit(this.value)'><i class='fa fa-eye' aria-hidden='true'> </i></button>
                    </td>";
